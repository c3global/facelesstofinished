...
**Backend** — new `/app/backend/appsumo_routes.py` (registered in server.py alongside admin/uploads):
  ...
  - Tier map (per Charity's live AppSumo listing — all tiers include both script engines): `{"1": ["base","shorts"], "2": ["base","shorts","studio"], "3": ["base","shorts","studio"]}` (JSON env override `APPSUMO_TIER_MAP`). NOTE: the listing's per-month render quotas (T2: 3 faceless/mo; T3: 10 faceless + 3 avatar/mo) and the Sprint/Avatar tier gates are NOT enforced in code — Charity explicitly instructed not to modify the existing Emergent-built render/scripts endpoints; only the tier→entitlement mapping is applied. Quota enforcement is a known follow-up if she asks for it.
