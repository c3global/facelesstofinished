...
**Backend** — new `/app/backend/appsumo_routes.py` (registered in server.py alongside admin/uploads):
  ...
  - Tier map: `APPSUMO_TIER_MAP` env (JSON), default `{"1": ["base"], "2": ["base","shorts"], "3": ["base","shorts","studio"]}`.
...
**Env vars to set in production** (Deployment → Environment):
  - `APPSUMO_WEBHOOK_TOKEN` — the `as_...` value in the webhook URL saved in the Partner Portal (endpoint is disabled while unset)
  - `APPSUMO_CLIENT_ID` / `APPSUMO_CLIENT_SECRET` — auto-generated by AppSumo after both URLs validate (redeem returns a friendly 503 until set)
  - `APPSUMO_API_KEY` — Partner Portal private key; enables HMAC verification (optional until the final key is issued on submission)
  - `APPSUMO_REDIRECT_URI` — defaults to `https://faceless48.c3global.co/redeem`; must byte-for-byte match the portal
  - `APPSUMO_TIER_MAP` — optional JSON override when tiers change
