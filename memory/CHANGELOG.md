...
- **Cost implication for Faceless launch**: earlier assumption that Sora 2 would replace Google Veo 3 Fast is REVERSED after the fix. For 9:16 vertical (Charity's primary short-form output), Sora 2 requires the Pro tier at $0.30-0.50/sec vs Veo 3 Fast at $0.15/sec — Veo is HALF the cost. Sora 2 is only cheaper for 16:9 landscape at $0.10/sec vs Veo Fast $0.15/sec. Decision (locked): keep Veo 3 Fast as the default in-app engine, Sora 2 remains admin-only test lane + optional Pro Plus feature for BYOK users who prefer OpenAI. Roadmap "Cinematic Faceless" blurb updated to reflect this.
